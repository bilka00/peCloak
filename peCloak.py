...
def get_code_cave (pe, skip_cave_search):
	...
	if code_cave_virtual_offset == 0:
		...
		code_cave_section = randstring(5)
...
def save_cloaked_pe(pe, file):
    #try:
		...
		pe.write(filename=fname) # MODIFIED WRITE FUNCTION IN PEFILE!!!
...
def add_fill_instructions(limit):
	...
	while (num_fill_instructions > 0):
		fill_instruction += get_one_trash_instruction()
...
def build_decoder(pe, encoder, section, decode_start, decode_end):
	...
	'''
	decode_instructions = {
								"ADD":"sub byte[ds:eax], ", # add encode w/ corresponding decoder ==> SUB BYTE PTR DS:[EAX]
								"SUB":"add byte[ds:eax], ",	# sub encode w/ corresponding add decoder ==> ADD BYTE PTR DS:[EAX]
								"XOR":"xor byte[ds:eax], "
							}
	decoder = []
	for i in encoder:
	...
	with open("unpack.asm", "w") as f:
		f.write(asm)
	os.system(os.getcwd()+r"\fasm\FASM.EXE unpack.asm")
					 
	return open("unpack.bin", "rb").read()
			...
			for section in sections:	
				...
				if section:
					...
					if encode_length == 0:
						...
						decode_end = section_start + encode_offset + encode_length
...
'''
def build_new_entry_jump(pe, new_entry_address):
	imports = {}
	for entry in pe.DIRECTORY_ENTRY_IMPORT:
		for imp in entry.imports:
			imports[imp.name] = imp.address
	asm = Template(open("jmp_to_original.tpl.asm", "r").read()).generate(
        imports=imports,
        go=new_entry_address,
		peCloak=peCloak,
		trash_max_count=5
    )
	with open("jmp_to_original.asm", "w") as f:
		f.write(asm)
	os.system(os.getcwd()+r"\fasm\FASM.EXE jmp_to_original.asm")
	return open("jmp_to_original.bin", "rb").read()
...
def main(argv):
	...
	restore_execution_flow = build_new_entry_jump(pe, new_entry_address)
